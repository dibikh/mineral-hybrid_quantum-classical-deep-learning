...
def main():
    ...
    print(f"[Hybrid] Train={len(X_train)}  Val={len(X_val)}  Test={len(X_test)}  n_features={n_features}")
    ...
    ds = torch.utils.data.TensorDataset(xtr, xtr, ytr)  # x_classical=x_quantum
    dl = torch.utils.data.DataLoader(
        ds, batch_size=cfg["hybrid"]["batch_size"], shuffle=True
    )
    ...
    print(f"[OK] Hybrid model  → {model_path}")
